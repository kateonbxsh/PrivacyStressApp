...
Smart city mobility systems increasingly rely on pervasive sensing,
...
locally and exchanges model updates rather than raw records; when
combined with clipping and Differential Privacy (DP), it can reduce
inference risk while preserving useful model performance in sensitive
medical and mobility settings~\cite{adnan2022}. These methods support a
decentralized architecture, but conventional FL still tends to merge
personal and shared learning into a single model.
...
\begin{table}[ht]
    \centering
    \caption{Privacy boundary of the Cloud--MEC--Device architecture.}
    \label{tab:privacy_boundary}
    \scriptsize
...
published, or stored outside the phone. Before transmission, the shared
update can still be clipped and noised:
\begin{equation}
    \tilde{\mathbf{w}}_{s,i} =
...
\begin{table}[ht]
    \centering
    \caption{Personalized hierarchical FL protocol.}
    \label{alg:pfl}
    \scriptsize
    \renewcommand{\arraystretch}{1.15}
    \begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}X}
        \toprule
        \textbf{Algorithm 1: Personalized hierarchical federated learning} \\
        \midrule
        \textbf{Input:} global shared model
        $\mathbf{w}_{s,\mathrm{cloud}}$, MEC region $r$, phone user $i$,
        local dataset $\mathcal{D}_i$. \\
        \textbf{1. Cloud initialization:} maintain
        $\mathbf{w}_{s,\mathrm{cloud}}$ from regional shared model
        submissions. \\
        \textbf{2. MEC initialization:} when a MEC node for region $r$
        starts, request $\mathbf{w}_{s,r}$ from the cloud; if no regional
        model exists, initialize $\mathbf{w}_{s,r} \leftarrow
        \mathbf{w}_{s,\mathrm{cloud}}$. \\
        \textbf{3. Phone initialization:} request $\mathbf{w}_{s,r}$
        from the nearest MEC; if unavailable, request cloud regional
        fallback; if unavailable, use a local default. Set
        $\mathbf{v}_i \leftarrow \mathbf{0}$. \\
        \textbf{4. Local inference:} compute
        $\mathbf{w}_i=\mathbf{w}_{s,r}+\mathbf{v}_i$ and predict
        discomfort from local profile, physiology, check-in context, and
        MEC situational context. \\
        \textbf{5. Local training:} update $\mathbf{w}_{s,r}$ and
        $\mathbf{v}_i$ using separate learning rates on
        $\nabla\mathcal{L}_i(\mathbf{w}_{s,r}+\mathbf{v}_i)$. \\
        \textbf{6. Phone-to-MEC upload:} transmit only
        $\tilde{\mathbf{w}}_{s,i}$ and $n_i$; never transmit
        $\mathbf{v}_i$ or raw data. \\
        \textbf{7. MEC aggregation:} compute
        $\mathbf{w}_{s,\mathrm{region},r} =
        \sum_i(n_i/N)\tilde{\mathbf{w}}_{s,i}$. \\
        \textbf{8. MEC-to-cloud upload:} transmit only the regional
        shared model and aggregate metadata. \\
        \textbf{9. Region switch:} replace only the shared component
        with the new region's $\mathbf{w}_{s,r'}$ and preserve
        $\mathbf{v}_i$. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
local personalized training, parameter clipping, Gaussian noise
injection, and submission of protected shared updates to the server.
...
\begin{table*}[t]
    \centering
    \caption{Main communication channels and privacy role.}
    \label{tab:protocol}
    \scriptsize
...
\begin{table}[ht]
    \centering
    \caption{Functional validation of the implemented prototype.}
...
\begin{table}[ht]
    \centering
    \caption{Prototype demonstration results using synthetic and manual
    test data.}
...
profile data. The current prototype also does not yet implement secure
aggregation, Byzantine-robust aggregation, formal client attestation, or
a calibrated differential privacy accountant. MQTT is used for local
prototype communication and should be deployed with TLS, authentication,
and topic-level access control in a production setting. Finally, the
prototype data are synthetic or manually generated; real-world
validation requires an ethically approved study with ASD participants
or caregivers, carefully designed consent materials, and controlled
evaluation of prediction accuracy, latency, usability, and safety.
